...
impl Handler {
    ...
    fn take_screenshot(&self, rect: Option<Rect<f32, CSSPixel>>) -> WebDriverResult<String> {
        ...
        let rgb = RgbaImage::from_raw(img.width, img.height, img.get_first_frame().bytes.to_vec()).unwrap(); // Should we seperately consider Image Type for Screenshot scenario?
