...
impl HttpCache {
    ...
    pub fn refresh(
        ...
        if let Some(cached_resources) = self.entries.get_mut(&entry_key) {
            if let Some(cached_resource) = cached_resources.iter_mut().find(|cached_resource| {
                if let Some(cached_entity_tag) = cached_resource
                    .metadata
                    .headers
                    .lock()
                    .unwrap()
                    .get(header::ETAG)
                {
                    if let Some(request_entity_tag) = request.headers.get(header::IF_NONE_MATCH) {
                        return request_entity_tag == cached_entity_tag;
                    }
                }
                false
            }) {
                let mut constructed_response = if let Some(range_spec) =
