...
use script_layout_interface::{
    IFrameSizes, ImageAnimateRegisterItem, ImageAnimationAction, LayoutImageAnimateHelper, PendingImage, PendingImageState
};
...
pub struct LayoutContext<'a> {
    ...
    pub register_image_animations: Mutex<Vec<ImageAnimationAction>>,
}
...
impl LayoutContext<'_> {
    ...
    pub fn get_frame_from_image(
        ...
        img.map(|image| {
            self.image_animation_helper
                .get_image_frame_from_image(&image, node, url)
        })
    }
    ...
    pub fn get_frame_key_from_image(
        ...
        img.map(|image| {
            self.image_animation_helper
                .get_image_frame_key_from_image(&image, node,url)
        })?
    }

    pub fn should_be_register(&self, node: OpaqueNode, img: Option<Arc<ImageContainer>>, url: Option<ServoUrl>) -> bool {
        if let Some(img) = img {
            if img.is_animate {
                !self.image_animation_helper.entry_exist(node,url)
            } else {
                false
            }
        } else {
        ...
        match self.get_or_request_image_or_meta(node, url.clone(), use_placeholder) {
            Some(ImageOrMetadataAvailable::ImageAvailable { image, .. }) => {
                let image_frame = self
                    .get_frame_from_image(Some(image.clone()), node,Some(url.clone()))
                    .unwrap(); // (Ray)TODO: better handle unwrap.
                if self.should_be_register(node, Some(image.clone()), Some(url.clone())) {
                    self.register_image_animations
                        .lock()
                        .push(ImageAnimationAction::Register(ImageAnimateRegisterItem {
                            node,
                            url: Some(url.clone()),
                            image: image.clone(),
                        }));
