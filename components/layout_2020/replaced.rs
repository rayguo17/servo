...
impl ReplacedContents {
            ...
            if let Some((image, natural_size_in_dots)) = element.as_image() {
                (
                    ReplacedContentKind::Image(
                        context
                            .get_frame_key_from_image(image, element.opaque(), None)// (Ray)TODO: Should get the url from element.
    ...
    pub fn from_image_url<'dom>(
        ...
        if let ComputedUrl::Valid(image_url) = image_url {
            ...
            let image_frame = context.get_frame_key_from_image(image, element.opaque(),Some(image_url.clone().into()));
            return Some(Self {
