...
pub struct ReflowResult {
    ...
    /// The list of action need to be done for Image with animation.
    pub animated_image_actions: Vec<ImageAnimationAction>,
}
...
pub struct LayoutImageAnimateHelper {
    pub mapping: HashMap<(OpaqueNode,Option<ServoUrl>), usize>, //node -> active index mapping.
}
...
impl LayoutImageAnimateHelper {
    ...
    pub fn get_image_frame_from_image(
        ...
        if image.is_animate {
            ...
            if let Some(frame_id) = self.mapping.get(&(node_id,url)) {
    ...
    pub fn entry_exist(&self, node_id: OpaqueNode, url: Option<ServoUrl>) -> bool {
        self.mapping.iter().for_each(|(k, v)| {
            println!("Node: {:?}, value: {:?}", k, v);
        });
        self.mapping.contains_key(&(node_id,url))
    }
}
...
/*
    ...
    we should check whether we should register or cancel the animation for this node.
...
#[derive(Debug)]
pub struct  ImageAnimationCancelItem { // aggregate try cancel item.
    pub node: OpaqueNode,
}
#[derive(Debug)]
